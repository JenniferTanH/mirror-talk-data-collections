...
laughs = ['haha', 'hahaha']
NAME = ["Kanye West", "West", "MR. WEST", "KW", "A", "Kanye", "WEST"]
Interviewee = "Kanye"
...
def LineSeperatedProcess(datafile, csvfile):
    ...
    with open("data/Line-Seperated/" + datafile, 'r') as f:
        name = "Interviewer"
        while 1:
            line = f.readline()
...
def TwoLineProcess(datafile, csvfile):
    csv_file = open("csv/" + csvfile, 'w')
    writer = csv.writer(csv_file)
    writer.writerow(["name", "line"])

    with open("data/Two-Line-Seperated/" + datafile, 'r') as f:
        while 1:
            line = f.readline()
...
def TimeLineSperated(datafile, csvfile):
    csv_file = open("csv/" + csvfile, 'w')
    writer = csv.writer(csv_file)
    writer.writerow(["name", "line"])
...
def SpecialHandle():
    csv_file = open("csv/" + "L.csv", 'w')
    writer = csv.writer(csv_file)
    writer.writerow(["name", "line"])

    with open("data/Special/Joe_Rogan_to_Kanye.txt", 'r') as f:
        name = "Interviewer"
        curr = ""
        while 1:
            line = f.readline()
            ...
            if len(line) < 2:
                if name == "Interviewer":
                    writer.writerow([name, LineProcess(curr)])
                    name = "Kanye"
                    curr = ""
                else:
                    writer.writerow([name, LineProcess(curr)])
...
def main():
    ...
    two_line_seperated_file = os.listdir("data/Two-Line-Seperated")
    for file in two_line_seperated_file:
        TwoLineProcess(file, file.replace(".txt", ".csv"))
